...
export function LiveAgentFeed() {
  ...
  useEffect(() => {
    // Swap the deterministic SEED_ROWS for random data in a frame callback, so
    // the server-rendered markup paints first (no hydration mismatch) without a
...
export function LiveAgentCounter() {
  ...
  return (
    <span style={{
      ...
      {/* `count` starts at 3847 and the locale is pinned, so the server render
          and the client's first paint agree — no mount guard needed. */}
