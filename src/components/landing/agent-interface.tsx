...
function HeatCell({ level, animDelay }: { level: number; animDelay: number }) {
  ...
  useEffect(() => { const t = setTimeout(() => setVisible(true), animDelay);

 return () => clearTimeout(t) }, [animDelay])
...
function ReviewLine({ item, delay }: { item: (typeof ALL_REVIEW_LINES)[0]; delay: number }) {
  ...
  useEffect(() => { const t = setTimeout(() => setVisible(true), delay);

 return () => clearTimeout(t) }, [delay])
...
export function AgentInterface({ revealDelay = 0 }: { revealDelay?: number }) {
  ...
  // Cursor blink
  useEffect(() => { const t = setInterval(() => setCursor(c => !c), 530);
