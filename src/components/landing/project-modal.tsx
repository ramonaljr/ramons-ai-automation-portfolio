    ...
    >
      ...
      >
        ...
        <div className="p-7 sm:p-9 pb-5">
          <div className="flex items-start justify-between gap-4">
            <div className="flex flex-wrap items-center gap-2">
              {cs.platform && (
                <span className="rounded-full border border-black/15 bg-black/[0.04] px-3 py-1 font-mono text-[12px] tracking-wide text-black/70">
                  {cs.platform}
                </span>
              )}
              {cs.categories?.map(c => (
                <span key={c} className="rounded-full border border-black/10 px-3 py-1 font-mono text-[12px] tracking-wide text-black/70">
                  {c}
                </span>
              ))}
              {cs.sample && (
                <span className="rounded-full border border-amber-600/30 bg-amber-500/[0.08] px-3 py-1 font-mono text-[11px] tracking-widest text-amber-700/80">
                  SAMPLE
                </span>
              )}
            </div>
            ...
            <button
              ...
              onClick={onClose}
              aria-label="Close"
              className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-black/10 text-black/70 transition-all hover:border-black/25 hover:text-black"
            >
          ...
          <p className="mt-3 max-w-2xl text-[14px] leading-relaxed text-black/68">{cs.description}</p>
        </div>
        ...
        <div className="flex gap-1 overflow-x-auto border-y border-black/[0.07] px-7 sm:px-9">
          {TABS.map(t => (
            <button
              ...
              type="button"
              onClick={() => setTab(t)}
              className={`flex shrink-0 items-center gap-2 border-b-2 px-3 py-4 text-[14px] transition-colors ${
                tab === t
                  ? "border-[#111] text-[#111]"
                  : "border-transparent text-black/68 hover:text-black/70"
              }`}
            >
              <Ico d={TAB_ICON[t]} />
              {label(t)}
            ...
            <>
              <div className="relative overflow-hidden rounded-xl border border-black/[0.07] bg-white">
                <button
                  type="button"
                  onClick={() => setZoom(z => !z)}
                  className="absolute right-3 top-3 z-10 inline-flex items-center gap-2 rounded-lg border border-black/10 bg-white/90 px-3 py-2 font-mono text-[12px] tracking-wide text-black/70 backdrop-blur transition-all hover:border-black/25 hover:text-black"
                >
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-[1.6fr_1fr]">
                <div className="rounded-xl border border-black/[0.07] bg-white/60 p-5">
                  <p className="font-mono text-[11px] tracking-widest text-black/68">
                    WORKFLOW LOGIC SUMMARY
                  </p>
                  <p className="mt-2.5 text-[14px] leading-relaxed text-black/72">{cs.logicSummary}</p>
                </div>
                {cs.keyOutcome && (
                  <div className="rounded-xl border border-emerald-600/20 bg-emerald-500/[0.06] p-5">
                    <p className="font-mono text-[11px] tracking-widest text-emerald-800/60">KEY OUTCOME</p>
                    <p
                    ...
                    <p className="mt-1 text-[14px] text-emerald-900/55">{cs.keyOutcome.label}</p>
                  </div>
                )}
              </div>
              ...
              {cs.integrations && (
                <>
                  <p className="mt-6 font-mono text-[11px] tracking-widest text-black/68">
                    ACTIVE INTEGRATIONS IN WORKFLOW:
                  </p>
                  <div className="mt-3 flex flex-wrap gap-2">
                    {cs.integrations.map(i => (
                      <span key={i} className="rounded-lg border border-black/[0.09] bg-white px-3.5 py-2 font-mono text-[13px] text-black/70">
                        {i}
                      </span>
                    ))}
                  </div>
          ...
          {tab === "Logic & Steps" && (
            <div className="grid gap-4 md:grid-cols-2">
              <div className="rounded-xl border border-rose-600/15 bg-rose-500/[0.04] p-6">
                <p className="font-mono text-[12px] tracking-widest text-rose-800/70">OPERATIONAL PROBLEM</p>
                <p className="mt-3 text-[14px] leading-relaxed text-black/72">{cs.problem}</p>
              </div>
              <div className="rounded-xl border border-emerald-600/20 bg-emerald-500/[0.05] p-6">
                <p className="font-mono text-[12px] tracking-widest text-emerald-800/70">ENGINEERED SOLUTION</p>
                <p className="mt-3 text-[14px] leading-relaxed text-black/72">{cs.solution}</p>
              </div>
            </div>
          )}
          ...
          {tab === "Impact & Metrics" && (
            <>
              <div className="rounded-xl border border-emerald-600/20 bg-emerald-500/[0.06] p-6 sm:p-7">
                <p className="font-mono text-[12px] tracking-widest text-emerald-800/70">PRIMARY HIGHLIGHT</p>
                <p
                ...
                <p className="mt-3 text-[14px] leading-relaxed text-black/68">{cs.impactHighlightDesc}</p>
              </div>

              {cs.roi && (
                <>
                  <p className="mt-7 font-mono text-[11px] tracking-widest text-black/68">QUANTIFIED ROI:</p>
                  <div className="mt-3 grid gap-4 sm:grid-cols-3">
                    {cs.roi.map(r => (
                      <div key={r.label} className="rounded-xl border border-black/[0.07] bg-white/60 p-6 text-center">
                        ...
                        <p className="mt-2 text-[13px] text-black/70">{r.label}</p>
                      </div>
                    ))}
                  </div>
                </>
          ...
          {tab === "Fail-Safes" && (
            <>
              <div className="rounded-xl border border-emerald-600/20 bg-emerald-500/[0.05] p-6 sm:p-7">
                ...
                <p className="mt-3 text-[14px] leading-relaxed text-black/68">{cs.failsafeDesc}</p>
              </div>
              ...
              <div className="mt-4 grid gap-4 md:grid-cols-2">
                {cs.failsafes?.map(f => (
                  <div key={f.title} className="rounded-xl border border-black/[0.07] bg-white/60 p-6">
                    ...
                    <p className="mt-2 text-[14px] leading-relaxed text-black/68">{f.desc}</p>
                  </div>
                ))}
              </div>
            </>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 border-t border-black/[0.07] p-7 sm:px-9">
          <p className="font-mono text-[13px] text-black/68">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {cs.videoUrl ? (
              <a
                ...
                className="inline-flex items-center gap-2 rounded-full border border-black/12 px-4 py-2.5 text-[13px] text-black/72 transition-all hover:border-black/30 hover:bg-black/[0.03] hover:text-black"
              >
              ...
              <span
                className="inline-flex cursor-default items-center gap-2 rounded-full border border-dashed border-black/12 px-4 py-2.5 text-[13px] text-black/72"
            ...
            <a
              href="/contact"
              className="inline-flex items-center gap-2.5 rounded-full bg-[#111] px-5 py-2.5 text-[13px] tracking-wide text-white transition-colors hover:bg-black"
            >
